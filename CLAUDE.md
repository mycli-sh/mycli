...
- JWT credentials stored in OS keyring with file fallback (`~/.my/credentials.json`); API tokens are read from `MY_API_TOKEN`. Setting `MY_NO_KEYRING` skips the keyring entirely and uses the file store (for headless/CI machines without a secure keyring)
- JWT sessions use a sliding 60-day window: `/v1/auth/refresh` rotates the refresh token AND bumps `sessions.expires_at` to `now() + 60d`. Refresh tokens are single-use; the immediately-previous token stays valid for a short reuse grace (`authservice.RefreshTokenGrace`) so a concurrent/duplicate refresh isn't rejected. On the CLI side, refresh is serialized both in-process (a package-global mutex) and across parallel `my` processes (an advisory file lock at `~/.my/refresh.lock`), throttled to at most once per `minRefreshInterval` (15m, the access-token lifetime), and the reactive 401 handler only clears credentials on a definitive auth rejection (all in `cli/internal/client`). The CLI fires a silent background refresh from root `PersistentPreRun` (`cli/internal/auth/background.go`) when `LastRefreshedAt` is older than 7 days, with a 30-min post-login grace; API tokens skip this path entirely
...
- Releases go through **one atomic endpoint**: `POST /v1/releases` publishes every library in a manifest inside a single DB transaction. Requests carry a per-library `content_sha256` (see `pkg/spec.LibraryReleaseHash` — the CLI marshals each spec via `CanonicalSpecBytes` and hashes those exact bytes; the server hashes what it received without re-marshaling, so canonical drift can't produce a false mismatch). Idempotency is four-way per library: no row → create; matching hash → `idempotent` (no work done, `latest_version` untouched); mismatched hash → 409 `RELEASE_CONTENT_MISMATCH`; legacy pre-migration row with NULL hash → 409 `RELEASE_EXISTS`. A tag strictly older than `latest_version` → 409 `RELEASE_STALE`. The CLI validates the manifest and every spec *before* creating the git tag, and `--push` reconciles the local tag with origin (checks remote SHA, refuses to overwrite, falls through to push on network error) so partial-failure retries are safe.
- Authenticated request bodies are capped at 256 KiB by default, 4 MiB on `POST /v1/releases` (`api/internal/middleware/bodylimit.go`). CLI pre-validates release payloads against the same limit (`cli/internal/client/client.MaxReleaseBodyBytes`) — keep the two in sync
