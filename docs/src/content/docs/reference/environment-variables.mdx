...
description: CLI and API server environment variable reference.
---

## CLI environment variables

These affect the `my` CLI on the user's machine.

| Variable | Description |
|----------|-------------|
| `MY_API_TOKEN` | Use an API token (`myc_…`) as the Bearer credential. The CLI skips magic-link login and JWT refresh entirely. Mint a token with `my cli token create`. |
| `MY_PROFILE` | Override the [active profile](/guides/profiles/) for this invocation. Takes priority over `~/.my/config.json`; falls back to `default` if neither is set. |
