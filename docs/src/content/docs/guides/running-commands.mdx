...
Libraries can have aliases, so `my k8s logs` may work as a shorthand for `my kubernetes logs`. Individual commands can also have aliases — for example, `my k pf` as a shorthand for `my kubernetes port-forward`. Use `my <library> --help` to see available commands in a library.
