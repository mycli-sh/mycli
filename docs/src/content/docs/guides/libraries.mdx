...
Registry installs are scoped to your **active [profile](/guides/profiles/)** on the server. With no profile explicitly set, the CLI uses the `default` profile (auto-created on signup). Switch profiles with `my cli profile set <slug>`, or target a non-active profile for one operation with the `--profile` flag:
...
To refresh the cache without (un)installing anything, run `my library sync`:

```bash
my library sync                  # active profile
my library sync --profile work   # a specific profile
my library sync --all            # every profile you own (useful before going offline)
```
...
<LinkCard title="library install" description="Install from the registry." href="/reference/library/install/" />
<LinkCard title="Profiles" description="Group libraries by workflow." href="/guides/profiles/" />
